...
<div class="container border border-dark">
	...
	<div class="btn-group ">
		<%
			if(curPage >1){
		%>
			<a href="<%=request.getContextPath()%>/search/filmSerachAction.jsp?curPage=<%=curPage-1%>&category=<%=category %>&rating=<%=rating %>&price=<%=price%>&length=<%=length%>&title=<%=title%>&actor=<%=actor%>" class="btn btn-outline-info">이전</a>
		<%
			}
		%>
		<!-- 다음 버튼 -->
		<%
		 	if(curPage < lastPage){
		 %>
		 	<a href="<%=request.getContextPath()%>/search/filmSerachAction.jsp?curPage=<%=curPage+1%>&category=<%=category%>&rating=<%=rating%>&price=<%=price%>&length=<%=length%>&title=<%=title%>&actor=<%=actor%>" class="btn btn-outline-info">다음</a>
		 <% 
		 	}
